...
class _TahunAjaranScreenState extends State<TahunAjaranScreen> {
  ...
  Future<void> fetchTahun() async {
    final response = await http.get(
      Uri.parse("${ApiConfig.baseUrl}/tahun-ajaran"),
    );

    if (response.statusCode == 200) {
      List data = jsonDecode(response.body);
      setState(() {
        listTahun = data.map((e) => TahunAjaran.fromJson(e)).toList();
        isLoading = false;
      });
    }
  }
  ...
  Future<void> aktifkanTahun(int id) async {
    await http.put(
      Uri.parse("${ApiConfig.baseUrl}/tahun-ajaran/aktif/$id"),
    );
    fetchTahun();
  }
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text(
                      tahun.status == "aktif" ? "Aktif" : "Tidak Aktif",
                    ),
